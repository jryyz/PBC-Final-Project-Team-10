	...
	#進入小遊戲分支
		...
		if ask_messagebox == True:
			...
			if play_messagebox == True:
				...
				guess_entry = Entry(window4).pack(side = TOP)
				submit_button = Button(window4,width = 10,text = "確定",command = check_ab).pack(side = TOP)#交答案
				result_label = Label(window4,width = 10, text = " ",font = ('Mono',20,"bold")).pack(side = TOP)#回傳結果
